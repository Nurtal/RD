...
def write_OverviewReport(target, control, dataType, panelRange, date):
	"""
	IN PROGRESS
	...
	report_file = open("REPORT/"+str(target)+"_vs_"+str(control)+".tex", "w")
	header_template = open("REPORT/template/Overview_header.tex", "r")
	...
	report_file.write("\\includegraphics[scale=0.30]{\"IMAGES/"+str(target)+"_vs_"+str(control)+"_PCA2D\"} & \\includegraphics[scale=0.30]{\"IMAGES/"+str(target)+"_vs_"+str(control)+"_PCA3D\"} \\\\ \n")
	...
	report_file.write("\\includegraphics[scale=0.15]{\"IMAGES/"+str(target)+"_vs_"+str(control)+"_matrixCorrelation\"} \\\\ \n")
	...
	report_file.write("\\begin{center}\n")
	report_file.write("\\begin{tabular}{c c}\n")
	report_file.write("\\includegraphics[scale=0.20]{\"IMAGES/"+str(target)+"_vs_"+str(control)+"_PCA3D_variance1\"} & \\includegraphics[scale=0.20]{\"IMAGES/"+str(target)+"_vs_"+str(control)+"_PCA3D_variance2\"} \\\\ \n")
...
def write_ClassificationReport(dataType, panelRange, date, listOfAssociation):
	...
	header_template = open("REPORT/template/Classification_header.tex", "r")
	for line in header_template:
		report_file.write(line)
	header_template.close()
